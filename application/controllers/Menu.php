...
class Menu extends CI_Controller {
	...
	public function index($id=NULL)
	{
		...
		$id_menu = $this->komposisi_menu->get_menuid();
		$data['id_menu'] = ( $id!=NULL ) ? $id : $id_menu;
		$check_id = ( $id!=NULL ) ? $id : $id_menu;
		$data['nama_menu'] = $this->komposisi_menu->get_menu($check_id)->nama_menu;
		// var_dump($data['nama_menu']);
		// die();
	...
	public function ajax_list()
	{
		...
		foreach ($list as $menu) {
			...
			$row[] = '<a class="btn btn-sm btn-primary" href="javascript:void(0)" title="Edit" onclick="edit_menu('."'".$menu->id_menu."'".')"><i class="glyphicon glyphicon-pencil"></i> Edit</a>
				  ...
				  <a class="btn btn-sm btn-info" href="javascript:void(0)" title="Detail" onclick="detail('."'".$menu->id_menu."'".')"><i class="fas fa-info-circle"></i> Komposisi</a>';
	...
	public function ajax_list_komposisi($id)
	{
		$this->load->helper('url');
		...
		foreach ($list as $komposisi_menu) {
			$no++;
			$row = array();
			$row[] = '<input type="checkbox" class="data-check-komposisi" value="'.$komposisi_menu->id_composition.'">';
	...
	public function ajax_edit_komposisi($id_composition)
	{	
	...
	public function ajax_add_komposisi()
	{
		$this->_validate_komposisi();
	...
	public function ajax_update_komposisi()
	{
		$this->_validate_komposisi();
		$data = array(
				'id_menu' => $this->input->post('id_menu'),
	...
	public function ajax_delete_komposisi($id_composition)
	{
		$this->komposisi_menu->delete_by_id($id_composition);
		echo json_encode(array("status" => TRUE));
	}

	public function ajax_bulk_delete_komposisi()
	{
	...
	private function _validate_komposisi()
	{
		$data = array();
		$data['error_string'] = array();
		$data['inputerror'] = array();
